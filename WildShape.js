 ...
 * Lots of changes here, sprinkled all through the code. Most changes occur anywhere
 * that WS_API.FIELDS.SENSES.LIGHT_ATTRS or WS_API.DEFAULT_CONFIG.SENSES are used. However,
 * I also added (in several place) checks to fallback to the default config values if
 * something is undefined. This was necessary because of the Roll20 API not being happy
 * to receive undefined values.
*/
...
const WS_API = {
    ...
    DEFAULT_CONFIG : {
        ...
        SENSES: {
            ...
            night_vision_tint: "#434343",
            night_vision_effect: "None",
            night_vision_effect_distance: 5,
...
class WildShapeMenu extends WildMenu
{
    ...
    showEditSenses(shifterId = null, shapeId = null) {
        ...
        _.each(WS_API.FIELDS.SENSES.LIGHT_ATTRS, (attr) => {
            const currAttr = settings[WS_API.FIELDS.SENSES.ROOT][attr] === undefined ? WS_API.DEFAULT_CONFIG.SENSES[attr] : settings[WS_API.FIELDS.SENSES.ROOT][attr];
...
var WildShape = WildShape || (function() {
    ...
    const copySenses = (from, to) =>
    {
        ...
        {
            toSenses[attr] = fromSenses[attr] === undefined ? WS_API.DEFAULT_CONFIG.SENSES[attr] : fromSenses[attr];
        });
    ...
    const setTokenBarValues = (token, barData, controlledby) => {
        function setValue(barKey)
        {
            if (barData[barKey])
            {
                ...
                {
                    token.set("showplayers_" + barKey, true);
    ...
    async function getTargetCharacterData(shiftData, isTargetNpc, isTargetDefault) {
        ...
        if (config[WS_API.FIELDS.SENSES.ROOT][WS_API.FIELDS.SENSES.OVERRIDE])
        {
            ...
            else
            {
                // copy the defaults
                _.each(WS_API.FIELDS.SENSES.LIGHT_ATTRS, (attr) => {
                    senses[attr] = config[WS_API.FIELDS.SENSES.ROOT][attr] === undefined ? WS_API.DEFAULT_CONFIG.SENSES[attr] : config[WS_API.FIELDS.SENSES.ROOT][attr];
                });
                ...
                if (isTargetNpc)
                {
                    ...
                    if (targetSenses)
                    {
                        ...
                        if (hasDarkvision) {
                            senses.has_night_vision = true;
                            senses.night_vision_distance = visionSense[1];
                        }
    ...
    async function doShapeShift(shiftData) {
        ...
        if (targetData.senses)
        {
            _.each(WS_API.FIELDS.SENSES.LIGHT_ATTRS, function setLightAttr(attr) {
                if (attr == "night_vision_effect" && targetData.senses["night_vision_effect"] == "Dimming") {
                    let effectDistance = targetData.senses["night_vision_effect_distance"] || 0;
                    let visionDistance = targetData.senses["night_vision_distance"] || 0;
                    let d = effectDistance / (visionDistance);
                    let dimming = `${targetData.senses[attr]}_${isFinite(d) ? d : 0}`;
                    shiftData.token.set(attr, dimming);
                }
                else if (attr == "night_vision_effect_distance") {
                    // Do nothing...
                }
                else {
                    shiftData.token.set(attr, targetData.senses[attr] === undefined ? WS_API.DEFAULT_CONFIG.SENSES[attr] : targetData.senses[attr]);
                }
            });
        }
            ...
            else if (field)
            {
                ...
                if (newValue)
                {
                    if (newValue == WS_API.CMD.TOGGLE)
                        ...
                        senses[WS_API.FIELDS.SENSES.ROOT][field] = newValue === undefined ? "" : newValue;
                }
            }
        }
            ...
            if (field)
            {
                ...
                { 
                    ...
                    else if(field == WS_API.FIELDS.CHARACTER)
                    {
                        ...
                        {
                            UTILS.chatError(`Cannot find character [ ${ch(newValue)} ] in the journal`);
                            isValueSet = false;
                        }
                    }
